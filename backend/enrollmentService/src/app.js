...
app.get('/enrollment-service/health', (_, res) => res.status(200).json({ message: 'Enrollment service and Running' }));
...
app.use('/enrollment-service/api', routes);
...
app.listen(port, (err) => {
  if (!err) {
    logger.info(`Enrollment service successfully started on port ${port}`);
  }
});

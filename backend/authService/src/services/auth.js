...
export const verifyMailTemplate = async (email, verification_code) => {
  const replacements = {
    ...
    action_text: 'Confirm',
    disclaimer_text: "You've received this email because you have opted to participate in Y3S2 2023."
  };
  const subject = 'Y3S2 - Account Verification';
...
export const resetPasswordMailTemplate = async (email, verification_code) => {
  ...
  const subject = 'Y3S2 - Reset Account Password';
  await sendMail(email, 'call_to_action', replacements, subject);
  return true;
};
